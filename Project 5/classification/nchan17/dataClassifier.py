...
# enhancedFeatureExtractorDigit takes care of adding new binary features.
# In this implementation one of the main features is based around number of whitespace areas,
# for calculating we use clustering method that groups white areas together, this feature has the weight of 10.
# Another one recoveres relation between pixels and their intensity using some values of features and datum
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    # adds features based on number of white areas in matrix of pixels
    def clusterWhiteAreas(features):
        clusters = []
        for y_coord in range(DIGIT_DATUM_HEIGHT):
            for x_coord in range(DIGIT_DATUM_WIDTH):
                if features[(x_coord, y_coord)] == 0:
                    success = 0
                    for cluster in clusters:
                        if (x_coord, y_coord - 1) in cluster:
                            cluster.append((x_coord, y_coord))
                            success += 1
                        elif (x_coord + 1, y_coord) in cluster:
                            cluster.append((x_coord, y_coord))
                            success += 1
                        elif (x_coord, y_coord + 1) in cluster:
                            cluster.append((x_coord, y_coord))
                            success += 1
                        elif (x_coord - 1, y_coord) in cluster:
                            cluster.append((x_coord, y_coord))
                            success += 1
                    if success == 0:
                        clusters.append([(x_coord, y_coord)])
        numWhiteAreas = len(clusters)
        if numWhiteAreas == 1:
            for i in range(9):
                features[("numWhiteAreas", i, 1)] = 1
        elif numWhiteAreas == 2:
            for i in range(9):
                features[("numWhiteAreas", i, 2)] = 1
        elif numWhiteAreas == 3:
            for i in range(9):
                features[("numWhiteAreas", i, 3)] = 1

    # recoveres relation between pixels and their intensity using some values of features and datum
    def compareMorePixel(datum, features):
        for x_coor in range(DIGIT_DATUM_WIDTH):
            for y_coor in range(DIGIT_DATUM_HEIGHT):
                curr_ft = features[(x_coor, y_coor)]
                left_ft = features[(x_coor - 1, y_coor)]
                down_ft = features[(x_coor, y_coor - 1)]
                curr = datum.getPixel(x_coor, y_coor)
                left = datum.getPixel(x_coor - 1, y_coor)
                down = datum.getPixel(x_coor, y_coor - 1)
                if curr_ft > left_ft:
                    features[("left_ft", x_coor, y_coor)] = True
                else:
                    features[("left_ft", x_coor, y_coor)] = False
                if curr_ft > down_ft:
                    features[("down_ft", x_coor, y_coor)] = True
                else:
                    features[("down_ft", x_coor, y_coor)] = False
                if curr > down:
                    features[("down", x_coor, y_coor)] = True
                else:
                    features[("down", x_coor, y_coor)] = False
                if curr > left:
                    features[("left", x_coor, y_coor)] = True
                else:
                    features[("left", x_coor, y_coor)] = False
    clusterWhiteAreas(features)
    compareMorePixel(datum, features)
    return features
...
# enhancedPacmanFeatures takes care of adding new features 
# I this implementation the features are: distance to the nearest ghost,
# distance to the nearest food, number of foods, ghost, final score
# and final state (win or lose).
def enhancedPacmanFeatures(state, action):
    ...
    if state.getFood()[pos[0]][pos[1]]:
        closestFoodDist = 0

    features["closest food dist"] = closestFoodDist
    features["closest ghost dist"] = closestGhostDist
    features["num food"] = len(food)
    features["num ghosts"] = len(ghostPositions)
    features["win"] = currentGameState.isWin()
    features["lose"] = currentGameState.isLose()
    features["score"] = currentGameState.getScore()/100
    return features
